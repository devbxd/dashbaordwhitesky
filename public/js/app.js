...
let _stmtAllNames=[];
async function loadStmtNames(){if(_stmtAllNames.length)return;const[inv,tkt]=await Promise.all([api('GET','/api/invoices'),api('GET','/api/tickets')]);const names=new Set();inv.forEach(i=>{if(i.client_name)names.add(i.client_name);});tkt.forEach(t=>{if(t.passenger)names.add(t.passenger);});_stmtAllNames=[...names].sort();}
