...
// Rendering the document into a hidden iframe and rasterizing it with html2pdf.js
// (html2canvas) was tried here to work around Electron's print-to-PDF being unreliable —
// twice, it still clipped the page down to a corner of the content instead of the whole
// document. Rather than keep patching a rasterizer that can't be trusted, this goes back to
// the plain, predictable path: a print popup using the browser/OS print dialog, where
// "Save as PDF" is a standard destination. `mode`/`wide` are unused now but left in the
// call sites so nothing else needs to change if a real server-rendered PDF replaces this
// for a given document type later (see /api/invoices/:id/pdf for invoices).
function outputDoc(doc,filename,mode,wide){
...
// Replaces the browser's native confirm() with a dialog styled like the rest of the app —
// a native confirm() breaks the app's own visual language and can't be restyled or localized.
function confirmDialog(message,{title='Are you sure?',confirmLabel='Confirm',danger=true}={}){
  return new Promise(resolve=>{
    ...
    if(danger){okBtn.style.background='var(--danger)';okBtn.style.color='#fff';okBtn.style.borderColor='var(--danger)';}
    else{okBtn.style.background='';okBtn.style.color='';okBtn.style.borderColor='';}
    document.getElementById('confirm-icon').className='confirm-icon'+(danger?'':' info');
    openModal('modal-confirm');
    const cleanup=(result)=>{closeModal('modal-confirm');okBtn.removeEventListener('click',onOk);cancelBtn.removeEventListener('click',onCancel);resolve(result);};
...
const SKELETON_PAGE='<div class="skeleton-page"><div class="skeleton skeleton-block" style="width:180px;height:22px"></div><div class="skeleton-stats"><div class="skeleton skeleton-stat"></div><div class="skeleton skeleton-stat"></div><div class="skeleton skeleton-stat"></div><div class="skeleton skeleton-stat"></div></div><div class="skeleton skeleton-table"></div></div>';
// Retriggers the page-enter fade on #main-content every time its content changes —
...
${rpt.otherCurrencies&&rpt.otherCurrencies.length?`<div class="info-box"><i class="ti ti-info-circle"></i> Numbers below are in ${rpt.currency} only — you also have invoices in ${rpt.otherCurrencies.join(', ')}, see Reports for the full breakdown.</div>`:''}
<div class="stats-grid">
  <div class="stat-card"><div class="stat-icon" style="background:#e6f9ee"><i class="ti ti-cash" style="color:#1a7a3a"></i></div><div class="stat-label">Collected</div><div class="stat-value" style="color:#1a7a3a">${fmt(rpt.paid,rpt.currency)}</div><div class="stat-detail">${allInvoices.filter(i=>i.status==='paid').length} invoice(s)</div></div>
  <div class="stat-card"><div class="stat-icon" style="background:#fff4e0"><i class="ti ti-clock" style="color:#a05c00"></i></div><div class="stat-label">Pending</div><div class="stat-value" style="color:#a05c00">${fmt(rpt.pending,rpt.currency)}</div><div class="stat-detail">${allInvoices.filter(i=>i.status==='pending').length} invoice(s)</div></div>
...
${upcoming&&upcoming.length?`<div class="card"><div class="card-header"><span class="card-title"><i class="ti ti-alarm" style="vertical-align:-2px;margin-right:6px;color:#b3382c"></i>Coming up</span></div><div class="deadline-list">${upcomingHtml}</div></div>`:''}
...
async function deleteClient(id){if(!await confirmDialog('Delete this client?'))return;await api('DELETE',`/api/clients/${id}`);toast('Client deleted');showPage('clients');}
...
async function markUnpaid(id){if(!await confirmDialog('The associated payment will be deleted.',{title:'Mark this invoice as unpaid?'}))return;await api('PATCH',`/api/invoices/${id}/status`,{status:'pending'});toast('Invoice marked as unpaid','error');viewInvoice(id);}
async function deleteInvoice(id){if(!await confirmDialog('Delete this invoice?'))return;await api('DELETE',`/api/invoices/${id}`);toast('Invoice deleted');showPage('invoices');}
...
<div class="header-actions">
  ...
  <button class="btn-secondary" onclick="printInv('print')"><i class="ti ti-printer"></i> Print</button>
  <a class="btn-secondary" href="/api/invoices/${inv.id}/pdf" style="text-decoration:none;display:inline-flex;align-items:center;gap:6px"><i class="ti ti-file-download"></i> Download PDF</a>
</div></div>
...
${hist.payments&&hist.payments.length?`<div class="card"><div class="card-header"><span class="card-title"><i class="ti ti-history" style="vertical-align:-2px;margin-right:6px;color:#1A6FB5"></i>Payment history</span></div><table style="width:100%;border-collapse:collapse"><thead><tr><th style="text-align:left;font-size:11px;color:#888;padding:6px 4px">Date</th><th style="text-align:left;font-size:11px;color:#888;padding:6px 4px">Method</th><th style="text-align:left;font-size:11px;color:#888;padding:6px 4px">Reference</th><th style="text-align:right;font-size:11px;color:#888;padding:6px 4px">Amount</th><th style="padding:6px 4px"></th></tr></thead><tbody>${hist.payments.map(p=>`<tr><td style="padding:6px 4px;font-size:13px">${fmtDate(p.date)}</td><td style="padding:6px 4px"><span class="pay-method-badge">${p.method}</span></td><td style="padding:6px 4px;color:#aaa;font-size:12px">${p.reference||'—'}</td><td style="padding:6px 4px;text-align:right;font-weight:700;color:#1a7a3a">${fmt(p.amount,inv.currency)}</td><td style="padding:6px 4px;text-align:right"><a href="/api/payments/${p.id}/receipt-pdf" title="Download Receipt" style="color:#1A6FB5"><i class="ti ti-file-download"></i></a></td></tr>`).join('')}</tbody><tfoot><tr style="border-top:1.5px solid #eee"><td colspan="4" style="padding:8px 4px;font-weight:700">Balance due</td><td style="padding:8px 4px;text-align:right;font-weight:800;color:${hist.balance>0?'#b71c1c':'#1a7a3a'}">${fmt(hist.balance,inv.currency)}</td></tr></tfoot></table></div>`:''}
<div id="printable"><div class="inv-wrap card">
  <div class="inv-head">
    ...
    <div class="inv-head-right"><div class="inv-title">INVOICE</div><div class="inv-meta-grid"><span class="inv-meta-label">Invoice #:</span><span class="inv-meta-val">${inv.num}</span><span class="inv-meta-label">Invoice date:</span><span class="inv-meta-val">${fmtDate(inv.date)}</span><span class="inv-meta-label">Payment terms:</span><span class="inv-meta-val">${fmtDate(inv.due_date)}</span></div></div>
  </div>
  <div class="inv-bill">
  ...
  <div class="inv-pax"><table class="inv-pax-table">
    ...
    <tbody>${rows.length===0?`<tr><td colspan="6" style="text-align:center;color:#bbb;padding:1.5rem">No rows</td></tr>`:rows.map(r=>`<tr><td><span class="inv-pnr">${r.pnr||'—'}</span></td><td>${r.destination||'—'}</td><td>${r.passenger||'—'}</td><td>${r.airlineRef||'—'}</td><td>${r.travel_date||'—'}</td><td>${inv.currency||'KWD'} ${Number(r.price).toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr>`).join('')}</tbody>
  </table></div>
...
function printInv(mode){
  const inv_content=document.getElementById('printable').innerHTML;
  const num=document.querySelector('.page-title')?.textContent||'Invoice';
  const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoice</title><style>
...
async function deleteQuote(id){if(!await confirmDialog('Delete this quote?'))return;await api('DELETE',`/api/quotes/${id}`);toast('Quote deleted');showPage('quotes');}
...
<div class="header-actions">
  ...
  <button class="btn-secondary" onclick="printQuote('print')"><i class="ti ti-printer"></i> Print</button>
  <a class="btn-secondary" href="/api/quotes/${qt.id}/pdf" style="text-decoration:none;display:inline-flex;align-items:center;gap:6px"><i class="ti ti-file-download"></i> Download PDF</a>
</div></div>
...
<div id="printable"><div class="inv-wrap card">
  ...
  <div class="inv-pax"><table class="inv-pax-table">
    <thead><tr><th>${L.colPnr}</th><th>${L.colDest}</th><th>${L.colPassenger}</th><th>${isCyber()?L.colAirline:(rows.some(r=>r.airline==='Hotel')?'Hotel':'Airline')}</th><th>Date</th><th>Price</th></tr></thead>
    <tbody>${rows.length===0?`<tr><td colspan="6" style="text-align:center;color:#bbb;padding:1.5rem">No rows</td></tr>`:rows.map(r=>`<tr><td><span class="inv-pnr">${r.pnr||'—'}</span></td><td>${r.destination||'—'}</td><td>${r.passenger||'—'}</td><td>${r.airlineRef||'—'}</td><td>${r.travel_date||'—'}</td><td>${qt.currency||'KWD'} ${Number(r.price).toLocaleString('en-US',{minimumFractionDigits:2})}</td></tr>`).join('')}</tbody>
  </table></div>
  <div class="inv-totals"><div class="inv-totals-inner">
...
function printQuote(mode){
  ...
  const num=document.querySelector('.page-title')?.textContent||'Quote';
  const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Quote</title><style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;color:#1a1a2e;background:#fff}
.inv-wrap{padding:0}
.inv-head{display:flex;justify-content:space-between;align-items:flex-start;padding:24px 30px 16px;border-bottom:3px solid #0a3258}
...
async function convertQuoteToInvoice(id){
  if(!await confirmDialog('This cannot be undone.',{title:'Convert this quote into an invoice?',danger:false,confirmLabel:'Convert'}))return;
...
async function deleteTicket(id){if(!await confirmDialog('Delete this ticket?'))return;await api('DELETE',`/api/tickets/${id}`);toast('Ticket deleted');showPage('tickets');}
...
<div class="card">
  ...
  <div class="form-grid2" style="gap:14px">
    ...
    <div class="form-group"><label class="form-label">Hotel Address</label><input class="form-input" id="htl-hotel-address" value="${h.hotel_address||''}" placeholder="Street, city"/></div>
    <div class="form-group"><label class="form-label">Hotel Phone</label><input class="form-input" id="htl-hotel-phone" value="${h.hotel_phone||''}"/></div>
...
async function saveHotel(){const type=document.getElementById('htl-type')?.value||'individual';const clientId=type==='company'?document.getElementById('htl-client-id')?.value||null:null;const passenger=document.getElementById('htl-passenger')?.value.trim()||'';const body={num:document.getElementById('htl-num')?.value.trim(),hotel_name:document.getElementById('htl-hotel-name')?.value.trim(),hotel_address:document.getElementById('htl-hotel-address')?.value.trim(),hotel_phone:document.getElementById('htl-hotel-phone')?.value.trim(),confirmation_num:document.getElementById('htl-confirmation')?.value.trim(),destination:document.getElementById('htl-destination')?.value.trim(),room_type:document.getElementById('htl-room')?.value.trim(),passenger,checkin_date:document.getElementById('htl-checkin')?.value,checkout_date:document.getElementById('htl-checkout')?.value,currency:document.getElementById('htl-currency')?.value||'KWD',net_price:document.getElementById('htl-net')?.value||0,selling_price:document.getElementById('htl-selling')?.value||0,status:document.getElementById('htl-status')?.value||'unpaid',notes:document.getElementById('htl-notes')?.value.trim(),booking_type:type,client_id:clientId};let r;if(_editHotelId){r=await api('PUT',`/api/hotels/${_editHotelId}`,body);toast('✅ Booking updated','success');}else{r=await api('POST','/api/hotels',body);toast('✅ Booking saved','success');}if(r&&r.error){toast(r.error,'error');return;}if(_editHotelId)viewHotel(_editHotelId);else showPage('hotels');}
async function deleteHotel(id){if(!await confirmDialog('Delete this hotel booking?'))return;await api('DELETE',`/api/hotels/${id}`);toast('Booking deleted');showPage('hotels');}

/* HOTEL VOUCHER — one per hotel booking; no separate data entry, just a printable
   confirmation slip generated from the booking already on file. */
async function pageHotelVouchers(mc){const hotels=await api('GET','/api/hotels');mc.innerHTML=`<div class="page-header"><div><div class="page-title">Hotel Voucher</div><div class="page-sub">${hotels.length} booking(s)</div></div></div><div class="card" style="padding:0;overflow:hidden"><div class="table-wrap"><table><thead><tr><th>#</th><th>Guest</th><th>Hotel</th><th>Check-in</th><th>Check-out</th><th>Room Type</th><th>Actions</th></tr></thead><tbody>${hotels.length===0?`<tr><td colspan="7"><div class="empty-state"><i class="ti ti-file-check"></i><h3>No hotel bookings</h3><p>Vouchers are generated from bookings created under Hotels.</p></div></td></tr>`:hotels.map(h=>`<tr><td style="font-weight:700">${h.num}</td><td>${h.passenger||'—'}</td><td>${h.hotel_name||'—'}</td><td>${fmtDate(h.checkin_date)}</td><td>${fmtDate(h.checkout_date)}</td><td>${h.room_type||'—'}</td><td class="actions-cell"><a class="action-btn" href="/api/hotels/${h.id}/voucher-pdf" title="Download Voucher"><i class="ti ti-file-download"></i> Voucher</a></td></tr>`).join('')}</tbody></table></div></div>`;}
...
async function deleteVisa(id){if(!await confirmDialog('Delete this visa application?'))return;await api('DELETE',`/api/visas/${id}`);toast('Visa deleted');showPage('visas');}
...
async function deleteGroup(id){if(!await confirmDialog('Delete this group?'))return;await api('DELETE',`/api/groups/${id}`);toast('Group deleted');showPage('groups');}
...
async function deletePassport(id){if(!await confirmDialog('Delete this passport record?'))return;await api('DELETE',`/api/passports/${id}`);toast('Passport deleted');showPage('passports');}
...
function printAllStmt(mode){
  ...
  const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>All Statements</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#222;background:#fff}.page{padding:20px 25px}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:12px;border-bottom:3px solid #0a3258;margin-bottom:16px}.header-left{display:flex;align-items:center;gap:14px}.company-info{font-size:10px;color:#555;line-height:1.6}.company-name{font-size:13px;font-weight:700;color:#0a3258;margin-bottom:2px}.stmt-title{text-align:right}.stmt-title h1{font-size:22px;font-weight:900;color:#0a3258;letter-spacing:.04em}.stmt-title p{font-size:10px;color:#888;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:10.5px}thead tr{background:#0a3258;color:#fff}th{padding:7px 8px;border:1px solid #0a3258;text-align:left;font-weight:700;font-size:10px;text-transform:uppercase;letter-spacing:.03em}td{padding:5px 8px;border:1px solid #ddd}tbody tr:nth-child(even) td{background:#f5f8fd}tfoot tr td{background:#e8eef5;font-weight:700;border:1px solid #bbb}.footer{margin-top:20px;padding-top:10px;border-top:1px solid #ddd;display:flex;justify-content:space-between;font-size:9.5px;color:#aaa}@media print{body{padding:0}.page{padding:10px 15px}button{display:none!important}}</style></head><body><div class="page"><div class="header"><div class="header-left">${logoHtml}<div class="company-info"><div class="company-name">${s.company_name||''}</div><div>${s.company_address||''}</div><div>P: ${s.company_phone_p||''} | M: ${s.company_phone_m||''}</div><div>${s.company_email||''}</div></div></div><div class="stmt-title"><h1>ALL STATEMENTS</h1><p>${fmtDate(from)} — ${fmtDate(to)}</p><p style="margin-top:6px;color:#555">Date: ${new Date().toLocaleDateString('en-GB')}</p></div></div>${content}<div class="footer"><span>${s.company_name||''} — ${s.company_email||''}</span><span>Generated on ${new Date().toLocaleString('en-GB')}</span></div></div><script>window.onload=()=>window.print()<\/script></body></html>`;
  outputDoc(doc,`All-Statements-${from}-to-${to}.pdf`,mode,true);
}
function printStmt(mode){const content=document.getElementById('stmt-printable')?.innerHTML;if(!content)return;const s=settings;const clientName=document.getElementById('stmt-client')?.selectedOptions[0]?.textContent||'Statement';const logoHtml=s.company_logo?`<img src="${s.company_logo}" style="height:60px;object-fit:contain" alt="Logo"/>`:`<div style="font-size:18px;font-weight:900;color:#0a3258">${s.company_name||(isCyber()?'M&S CYBER SYSTEMS':'✈ WHITE SKY')}</div>`;const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Statement of Account</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#222;background:#fff}.page{padding:20px 25px}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:12px;border-bottom:3px solid #0a3258;margin-bottom:16px}.header-left{display:flex;align-items:center;gap:14px}.company-info{font-size:10px;color:#555;line-height:1.6}.company-name{font-size:13px;font-weight:700;color:#0a3258;margin-bottom:2px}.stmt-title{text-align:right}.stmt-title h1{font-size:22px;font-weight:900;color:#0a3258;letter-spacing:.04em}.stmt-title p{font-size:10px;color:#888;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:10.5px}thead tr{background:#0a3258;color:#fff}th{padding:7px 8px;border:1px solid #0a3258;text-align:left;font-weight:700;font-size:10px;text-transform:uppercase;letter-spacing:.03em}td{padding:5px 8px;border:1px solid #ddd}tbody tr:nth-child(even) td{background:#f5f8fd}tfoot tr td{background:#e8eef5;font-weight:700;border:1px solid #bbb}.footer{margin-top:20px;padding-top:10px;border-top:1px solid #ddd;display:flex;justify-content:space-between;font-size:9.5px;color:#aaa}@media print{body{padding:0}.page{padding:10px 15px}button{display:none!important}}</style></head><body><div class="page"><div class="header"><div class="header-left">${logoHtml}<div class="company-info"><div class="company-name">${s.company_name||''}</div><div>${s.company_address||''}</div><div>P: ${s.company_phone_p||''} | M: ${s.company_phone_m||''}</div><div>${s.company_email||''}</div></div></div><div class="stmt-title"><h1>STATEMENT</h1><p>OF ACCOUNT</p><p style="margin-top:6px;color:#555">Date: ${new Date().toLocaleDateString('en-GB')}</p></div></div>${content}<div class="footer"><span>${s.company_name||''} — ${s.company_email||''}</span><span>Generated on ${new Date().toLocaleString('en-GB')}</span></div></div><script>window.onload=()=>window.print()<\/script></body></html>`;outputDoc(doc,`Statement-${clientName}.pdf`,mode,true);}
...
function printPersonStmt(mode){
  ...
  outputDoc(doc,`Statement-${personName}.pdf`,mode,true);
}
...
/* RECEIPTS — one per payment recorded against an invoice; no separate data entry, just a
   printable proof-of-payment for something already in the system. */
async function pageReceipts(mc){const list=await api('GET','/api/payments');const total=list.reduce((a,p)=>a+(parseFloat(p.amount)||0),0);mc.innerHTML=`<div class="page-header"><div><div class="page-title">Receipts</div><div class="page-sub">${list.length} payment(s) received — Total: <strong>${fmt(total)}</strong></div></div></div><div class="card" style="padding:0;overflow:hidden"><div class="table-wrap"><table><thead><tr><th>Date</th><th>Invoice #</th><th>Client</th><th>Method</th><th>Reference</th><th>Amount</th><th>Actions</th></tr></thead><tbody>${list.length===0?`<tr><td colspan="7"><div class="empty-state"><i class="ti ti-receipt"></i><h3>No payments received yet</h3><p>Receipts are generated from payments recorded on invoices.</p></div></td></tr>`:list.map(p=>`<tr><td>${fmtDate(p.date)}</td><td style="font-weight:700;cursor:pointer;color:#1A6FB5" onclick="viewInvoice(${p.invoice_id})">${p.invoice_num||'—'}</td><td>${p.client_name||'—'}</td><td><span class="pay-method-badge">${p.method||'—'}</span></td><td style="color:#aaa;font-size:12px">${p.reference||'—'}</td><td style="font-weight:700;color:#1a7a3a">${fmt(p.amount,p.currency)}</td><td class="actions-cell"><a class="action-btn" href="/api/payments/${p.id}/receipt-pdf" title="Download Receipt"><i class="ti ti-file-download"></i> Receipt</a></td></tr>`).join('')}</tbody></table></div></div>`;}
...
async function deleteCreditNote(id){if(!await confirmDialog('Delete this credit note?'))return;await api('DELETE',`/api/credit-notes/${id}`);toast('Credit note deleted');showPage('credit-notes');}
...
<div class="header-actions"><button class="btn-secondary" onclick="showPage('credit-notes')"><i class="ti ti-arrow-left"></i> Back</button><button class="btn-secondary" onclick="printCreditNote('print')"><i class="ti ti-printer"></i> Print</button><a class="btn-secondary" href="/api/credit-notes/${cn.id}/pdf" style="text-decoration:none;display:inline-flex;align-items:center;gap:6px"><i class="ti ti-file-download"></i> Download PDF</a></div></div>
<div id="printable"><div class="inv-wrap card">
  <div class="inv-head">
    <div class="inv-head-left"><div style="display:flex;flex-direction:column;align-items:flex-start;gap:8px">${logoHtml}<div class="inv-company-name">${s.company_name||''}</div></div></div>
...
function printCreditNote(mode){
  const content=document.getElementById('printable').innerHTML;
  const num=document.querySelector('.page-title')?.textContent||'Credit-Note';
  const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Credit Note</title><style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',Arial,sans-serif;font-size:12px;color:#1a1a2e;background:#fff}
.inv-wrap{padding:0}
.inv-head{display:flex;justify-content:space-between;align-items:flex-start;padding:24px 30px 16px;border-bottom:3px solid #0a3258}
...
async function loadReport(){
  ...
  const primaryList=list.filter(i=>(i.currency||'KWD')===rpt.currency);
  const total=primaryList.reduce((a,i)=>a+i.total,0);
  const otherCurrencyCount=list.length-primaryList.length;

  const months=Object.keys(rpt.byMonth||{}).sort();
  const maxMonth=Math.max(1,...months.map(m=>rpt.byMonth[m]));
  const monthNames=['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'];
  const chartHtml=months.length===0?`<div class="empty-state" style="padding:2rem"><i class="ti ti-chart-bar"></i><h3>No revenue data for this period</h3></div>`:`<div class="month-chart">${months.map(m=>{const val=rpt.byMonth[m]||0;const h=Math.round((val/maxMonth)*100);const[y,mo]=m.split('-');return`<div class="month-bar-wrap" title="${fmt(val,rpt.currency)}"><div class="month-val">${val>=1000?(val/1000).toFixed(1)+'k':Math.round(val)}</div><div class="month-bar" style="height:${Math.max(h,2)}%"></div><div class="month-label">${monthNames[parseInt(mo,10)-1]} ${y.slice(2)}</div></div>`;}).join('')}</div>`;

  const topClients=rpt.topClients||[];
  const topClientsHtml=topClients.length===0?'':`<table class="top-clients-table" style="width:100%;border-collapse:collapse">${topClients.map(([name,amt],idx)=>`<tr><td style="width:26px;color:#bbb;font-weight:700">#${idx+1}</td><td style="font-weight:600">${name}</td><td style="text-align:right;font-weight:700;color:#0a3258">${fmt(amt,rpt.currency)}</td></tr>`).join('')}</table>`;
...
async function toggleUserActive(id,active){
  if(!active&&!await confirmDialog('They will be blocked from signing in until you reactivate it.',{title:'Deactivate this account?'}))return;
...
async function deleteInvite(code){if(!await confirmDialog('Revoke this unused code?'))return;await api('DELETE',`/api/invites/${code}`);toast('Code revoked');showPage('admin');}
