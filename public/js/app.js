...
function outputDoc(doc,filename,mode){
  if(mode==='pdf'){
    const clean=doc.replace('<script>window.onload=()=>window.print()<\/script>','');
    const iframe=document.createElement('iframe');
    iframe.style.cssText='position:fixed;left:-99999px;top:0;width:900px;height:1px;border:0';
    document.body.appendChild(iframe);
    iframe.onload=()=>{
      html2pdf().set({margin:0,filename,image:{type:'jpeg',quality:.98},html2canvas:{scale:2,useCORS:true,windowWidth:900},jsPDF:{unit:'mm',format:'a4',orientation:'portrait'}}).from(iframe.contentDocument.body).save().catch(()=>toast('Could not generate the PDF','error')).finally(()=>iframe.remove());
    };
    iframe.srcdoc=clean;
  }else{
...
function printAllStmt(mode){
  ...
  outputDoc(doc,`All-Statements-${from}-to-${to}.pdf`,mode);
}
function printStmt(mode){const content=document.getElementById('stmt-printable')?.innerHTML;if(!content)return;const s=settings;const clientName=document.getElementById('stmt-client')?.selectedOptions[0]?.textContent||'Statement';const logoHtml=s.company_logo?`<img src="${s.company_logo}" style="height:60px;object-fit:contain" alt="Logo"/>`:`<div style="font-size:18px;font-weight:900;color:#0a3258">${s.company_name||(isCyber()?'M&S CYBER SYSTEMS':'✈ WHITE SKY')}</div>`;const doc=`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Statement of Account</title><style>*{box-sizing:border-box;margin:0;padding:0}body{font-family:Arial,sans-serif;font-size:11px;color:#222;background:#fff}.page{padding:20px 25px}.header{display:flex;justify-content:space-between;align-items:flex-start;padding-bottom:12px;border-bottom:3px solid #0a3258;margin-bottom:16px}.header-left{display:flex;align-items:center;gap:14px}.company-info{font-size:10px;color:#555;line-height:1.6}.company-name{font-size:13px;font-weight:700;color:#0a3258;margin-bottom:2px}.stmt-title{text-align:right}.stmt-title h1{font-size:22px;font-weight:900;color:#0a3258;letter-spacing:.04em}.stmt-title p{font-size:10px;color:#888;margin-top:2px}table{width:100%;border-collapse:collapse;font-size:10.5px}thead tr{background:#0a3258;color:#fff}th{padding:7px 8px;border:1px solid #0a3258;text-align:left;font-weight:700;font-size:10px;text-transform:uppercase;letter-spacing:.03em}td{padding:5px 8px;border:1px solid #ddd}tbody tr:nth-child(even) td{background:#f5f8fd}tfoot tr td{background:#e8eef5;font-weight:700;border:1px solid #bbb}.footer{margin-top:20px;padding-top:10px;border-top:1px solid #ddd;display:flex;justify-content:space-between;font-size:9.5px;color:#aaa}@media print{body{padding:0}.page{padding:10px 15px}button{display:none!important}}</style></head><body><div class="page"><div class="header"><div class="header-left">${logoHtml}<div class="company-info"><div class="company-name">${s.company_name||''}</div><div>${s.company_address||''}</div><div>P: ${s.company_phone_p||''} | M: ${s.company_phone_m||''}</div><div>${s.company_email||''}</div></div></div><div class="stmt-title"><h1>STATEMENT</h1><p>OF ACCOUNT</p><p style="margin-top:6px;color:#555">Date: ${new Date().toLocaleDateString('en-GB')}</p></div></div>${content}<div class="footer"><span>${s.company_name||''} — ${s.company_email||''}</span><span>Generated on ${new Date().toLocaleString('en-GB')}</span></div></div><script>window.onload=()=>window.print()<\/script></body></html>`;outputDoc(doc,`Statement-${clientName}.pdf`,mode);}
...
function printPersonStmt(mode){
  ...
  outputDoc(doc,`Statement-${personName}.pdf`,mode);
}
