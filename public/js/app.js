...
function invRowsHtml(list){if(!list.length)return`<tr><td colspan="8"><div class="empty-state"><i class="ti ti-file-off"></i><h3>No invoices</h3></div></td></tr>`;return list.map(i=>`<tr><td style="font-weight:700;cursor:pointer;color:#1A6FB5" onclick="viewInvoice(${i.id})">${i.num}</td><td>${i.client_name}</td><td>${fmtDate(i.date)}</td><td>${fmtDate(i.due_date)}</td><td style="font-weight:700">${fmt(i.total,i.currency)}</td><td>${statusBadge(i.status)}</td><td style="color:#aaa;font-size:12px">${i.owner_name||'—'}</td><td class="actions-cell"><button class="action-btn" onclick="viewInvoice(${i.id})"><i class="ti ti-eye"></i></button><button class="action-btn" onclick="editInvoice(${i.id})"><i class="ti ti-edit"></i></button><button class="action-btn" onclick="openPayModal(${i.id})"><i class="ti ti-check"></i></button><button class="action-btn danger" onclick="deleteInvoice(${i.id})"><i class="ti ti-trash"></i></button></td></tr>`).join('');}
