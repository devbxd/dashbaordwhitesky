...
// Print and PDF used to be one button that just called window.print() in a popup window —
// inside the Electron desktop app, "Save as PDF" from that native print dialog doesn't
// reliably produce a file. This renders the exact same document into a hidden iframe and
// converts it to a real .pdf with html2pdf.js instead, so downloading never depends on the
// OS print dialog. `doc` is the full HTML string already built for the print popup — reused
// as-is, minus the auto window.print() trigger it ends with.
function outputDoc(doc,filename,mode,wide){
  if(mode==='pdf'){
    const clean=doc.replace('<script>window.onload=()=>window.print()<\/script>','');
    // Placing this at an extreme offscreen coordinate (e.g. left:-99999px) made html2canvas
    // capture only a sliver of the page — some builds compute the capture box relative to the
    // parent document, and a huge negative offset breaks that math. Sitting at (0,0) with
    // opacity 0 and z-index -1 stays just as invisible without the giant coordinates.
    const w=wide?1300:900;
    const iframe=document.createElement('iframe');
    iframe.style.cssText=`position:fixed;left:0;top:0;width:${w}px;height:1200px;border:0;opacity:0;pointer-events:none;z-index:-1`;
    document.body.appendChild(iframe);
    iframe.onload=()=>{
      const body=iframe.contentDocument.body;
      iframe.style.height=Math.max(body.scrollHeight,1200)+'px';
      html2pdf().set({margin:0,filename,image:{type:'jpeg',quality:.98},html2canvas:{scale:2,useCORS:true,windowWidth:w},jsPDF:{unit:'mm',format:'a4',orientation:wide?'landscape':'portrait'}}).from(body).save().catch(()=>toast('Could not generate the PDF','error')).finally(()=>iframe.remove());
    };
    iframe.srcdoc=clean;
  }else{
...
<div class="header-actions">
  ...
  <button class="btn-secondary" onclick="printInv('pdf')"><i class="ti ti-file-download"></i> Download PDF</button>
</div></div>
...
<div class="header-actions">
  ...
  <button class="btn-secondary" onclick="printQuote('pdf')"><i class="ti ti-file-download"></i> Download PDF</button>
</div></div>
...
<div class="header-actions"><button class="btn-secondary" onclick="showPage('credit-notes')"><i class="ti ti-arrow-left"></i> Back</button><button class="btn-secondary" onclick="printCreditNote('print')"><i class="ti ti-printer"></i> Print</button><button class="btn-secondary" onclick="printCreditNote('pdf')"><i class="ti ti-file-download"></i> Download PDF</button></div></div>
<div id="printable"><div class="inv-wrap card">
  <div class="inv-head">
    <div class="inv-head-left"><div style="display:flex;flex-direction:column;align-items:flex-start;gap:8px">${logoHtml}<div class="inv-company-name">${s.company_name||''}</div></div></div>
